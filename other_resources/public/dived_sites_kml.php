...
if ($sql_data->num_rows > 0) {
  ...
  while($row = $sql_data->fetch_assoc()) {
    echo '<Placemark><name><![CDATA[ ' . $row['site_name'] . ' ]]></name><description><![CDATA[ ' . $row['dives'] . ' Dives ]]></description><Point><coordinates> ' . $row['east'] . ',' . $row['north'] . '</coordinates></Point></Placemark>';
	}
}
$conn->close();
echo '</Document></kml>';
